...
extern "C"
{
    ...
    int tg3json_parse_n(const char* data,
                        size_t len,
                        size_t depth_limit,
                        tg3json_value* out_value,
                        const char** out_error_pos);
    int tg3json_parse(const char* begin,
                      const char* end,
                      size_t depth_limit,
                      tg3json_value* out_value,
                      const char** out_error_pos);
    ...
    int tg3json_parse_n_opts(const char* data,
                             size_t len,
    ...
    const tg3json_value* tg3json_object_get(const tg3json_value* object, const char* key);
    const tg3json_value* tg3json_object_get_n(const tg3json_value* object, const char* key, size_t key_len);
    tg3json_value* tg3json_object_get_mut(tg3json_value* object, const char* key);
    tg3json_value* tg3json_object_get_mut_n(tg3json_value* object, const char* key, size_t key_len);
    const tg3json_object_entry* tg3json_object_at(const tg3json_value* object, size_t index);
    size_t tg3json_object_size(const tg3json_value* object);
    int tg3json_object_set_take_n(tg3json_value* object, const char* key, size_t key_len, tg3json_value* value);
    int tg3json_object_set_take(tg3json_value* object, const char* key, tg3json_value* value);
    int tg3json_object_set_copy_n(tg3json_value* object, const char* key, size_t key_len, const tg3json_value* value);
    int tg3json_object_set_copy(tg3json_value* object, const char* key, const tg3json_value* value);
...
static int tg3json__is_inf_bits(uint64_t bits) { return (bits & 0x7fffffffffffffffULL) == 0x7ff0000000000000ULL; }
...
static int tg3json__parse_f64_c(const char* start, const char* end, double* out)
{
    ...
    *out = v;
    ...
    if (exp10 != 0)
        v *= tg3json__pow10_ld(exp10);
    if (neg)
        v = -v;
    *out = (double)v;
    if (tg3json__is_inf_bits(tg3json__double_bits(*out)))
        return 0;
    return 1;
#endif
}
...
static char* tg3json__format_decimal_digits(char* out, const char* digits, int ndigits, int dec_exp, int negative)
{
    char* p = out;
    int i;
...
static int tg3json__same_f64(double a, double b) { return tg3json__double_bits(a) == tg3json__double_bits(b); }
...
static void tg3json__expand_short_plain_decimal(char* s)
{
    ...
    for (; p < e; ++p)
    {
        ...
        {
            if (ndigits < (int)(sizeof(digits) - 1))
                digits[ndigits++] = *p;
        }
    }
    if (int_digits == 0)
...
static char* tg3json__dtoa_c(double value, char* buf)
{
    ...
    {
        char* end = tg3json__format_decimal_digits(best, digits, ndigits, dec_e - ndigits + 1, negative);
        *end = '\0';
    ...
    for (i = ndigits - 1; i >= 1; --i)
    {
        ...
        char* end;
        double parsed = 0.0;
        end = tg3json__format_decimal_digits(candidate, digits, i, dec_e - i + 1, negative);
        *end = '\0';
        if (tg3json__parse_f64_c(candidate, end, &parsed) && tg3json__same_f64(parsed, value))
        {
...
static char* tg3json__strndup_local(const char* src, size_t len)
{
    ...
    if (!dst)
        return NULL;
    if (len > 0)
        TINYGLTF_JSON_MEMCPY(dst, src, len);
    dst[len] = '\0';
    return dst;
}
...
static void* tg3json__parser_alloc(tg3json__parser* parser, size_t size)
{
    void* ptr;
    if (!parser)
        return TINYGLTF_JSON_MALLOC(size);
    if (parser->max_single_alloc && size > parser->max_single_alloc)
        return NULL;
    if (parser->memory_budget && (size > parser->memory_budget || parser->allocated > parser->memory_budget - size))
    {
        return NULL;
    }
...
static int tg3json__reserve_bytes(void** ptr, size_t elem_size, size_t needed, size_t* capacity)
{
    void* new_ptr;
    size_t new_cap;
...
static int tg3json__reserve_bytes_parser(tg3json__parser* parser,
                                         void** ptr,
                                         size_t elem_size,
                                         size_t needed,
                                         size_t* capacity)
{
    ...
    if (parser)
    {
        ...
        if (parser->max_single_alloc && new_bytes > parser->max_single_alloc)
            return 0;
        if (parser->memory_budget && delta > 0
            && (delta > parser->memory_budget || parser->allocated > parser->memory_budget - delta))
        {
            return 0;
        }
    }
...
static int tg3json__buf_append(tg3json__buffer* buf, const char* src, size_t len)
{
    if (len == 0)
        return 1;
    if (!tg3json__reserve_bytes_parser(buf->parser, (void**)&buf->data, 1, buf->len + len + 1, &buf->cap))
    {
        return 0;
    }
...
{
    if (!tg3json__reserve_bytes_parser(buf->parser, (void**)&buf->data, 1, buf->len + 2, &buf->cap))
    {
        return 0;
    }
    buf->data[buf->len++] = c;
...
static int tg3json__parse_string_raw(tg3json__parser* parser, char** out_str, size_t* out_len)
{
    tg3json__buffer buf;
    const char* start;
    ...
    while (parser->cur < parser->end)
    {
        ...
        if (c == '"')
        {
            ...
            if (parser->max_string_length && final_len > parser->max_string_length)
                goto oom;
            if (!tg3json__buf_append(&buf, start, (size_t)(parser->cur - start)))
                goto oom;
            ++parser->cur;
            *out_str = buf.data;
            *out_len = buf.len;
        ...
        if (c == '\\')
        {
            ...
            if (parser->max_string_length && buf.len + pending_len > parser->max_string_length)
                goto oom;
            if (!tg3json__buf_append(&buf, start, (size_t)(parser->cur - start)))
                goto oom;
            ++parser->cur;
            ...
            switch (*parser->cur)
            {
                ...
                case '/':
                    ...
                    break;
                case 'b':
                    if (!tg3json__buf_putc(&buf, '\b'))
                        goto oom;
                    break;
                case 'f':
                    if (!tg3json__buf_putc(&buf, '\f'))
                        goto oom;
                    break;
                case 'n':
                    if (!tg3json__buf_putc(&buf, '\n'))
                ...
                case 'u':
                    {
                        if ((size_t)(parser->end - parser->cur) < 5)
                            break;
                        if (!tg3json__hex4(parser->cur + 1, &codepoint))
                            break;
                        parser->cur += 4;
                        if (codepoint >= 0xD800u && codepoint <= 0xDBFFu)
                        {
                            uint32_t low;
                            if ((size_t)(parser->end - parser->cur) < 7 || parser->cur[1] != '\\'
                                || parser->cur[2] != 'u')
                                break;
                            if (!tg3json__hex4(parser->cur + 3, &low))
                                break;
                        ...
                        if (!tg3json__append_utf8(&buf, codepoint))
                            goto oom;
                        break;
                    }
                default:
...
static int tg3json__parse_value(tg3json__parser* parser, size_t depth, tg3json_value* out_value);

static int tg3json__parse_int64_span(const char* start, const char* end, int64_t* out)
{
    const char* p = start;
    uint64_t value = 0;
...
static int tg3json__parse_array(tg3json__parser* parser, size_t depth, tg3json_value* out_value)
{
...
static int tg3json__parse_object(tg3json__parser* parser, size_t depth, tg3json_value* out_value)
{
    ...
    while (parser->cur < parser->end)
    {
        ...
        tg3json_value value;
        tg3json__init_value(&value);
        if (!tg3json__parse_string_raw(parser, &key, &key_len))
            goto fail;
        parser->cur = tg3json__skip_ws(parser->cur, parser->end);
        if (parser->cur >= parser->end || *parser->cur != ':')
        {
...
static int tg3json__parse_number(tg3json__parser* parser, tg3json_value* out_value)
{
    ...
    {
        if (!tg3json__parse_f64_c(start, p, &out_value->u.real))
            goto fail;
        if (parser->parse_float32)
            out_value->u.real = (double)(float)out_value->u.real;
...
static int tg3json__parse_value(tg3json__parser* parser, size_t depth, tg3json_value* out_value)
{
    ...
    switch (*parser->cur)
    {
        case '[': return tg3json__parse_array(parser, depth, out_value);
        case '{': return tg3json__parse_object(parser, depth, out_value);
        case '"':
            {
                char* str = NULL;
                size_t len = 0;
                if (!tg3json__parse_string_raw(parser, &str, &len))
                    return 0;
        ...
        case 'f':
            ...
            break;
        default: return tg3json__parse_number(parser, out_value);
    }
                         ...
                         const char** out_error_pos)
{
    ...
    {
        if (out_error_pos)
            *out_error_pos = parser.error ? parser.error : parser.cur;
                  ...
                  const char** out_error_pos)
{
    ...
    return tg3json_parse_n(begin, (size_t)(end - begin), depth_limit, out_value, out_error_pos);
}
...
void tg3json_value_free(tg3json_value* value)
{
    ...
    switch (value->type)
    {
        case TG3JSON_STRING: TINYGLTF_JSON_FREE(value->u.string.ptr); break;
        case TG3JSON_ARRAY:
...
void tg3json_value_init_null(tg3json_value* value) { tg3json__init_value(value); }
...
int tg3json_value_copy(tg3json_value* dst, const tg3json_value* src)
{
    ...
    switch (src->type)
    {
        case TG3JSON_NULL:   return 1;
        case TG3JSON_BOOL:   tg3json_value_init_bool(dst, src->u.boolean); return 1;
        case TG3JSON_INT:    tg3json_value_init_int(dst, src->u.integer); return 1;
        case TG3JSON_REAL:   tg3json_value_init_real(dst, src->u.real); return 1;
        case TG3JSON_STRING: return tg3json_value_init_string_n(dst, src->u.string.ptr, src->u.string.len);
        case TG3JSON_ARRAY:
        ...
        case TG3JSON_OBJECT:
            ...
            {
                if (!tg3json_object_set_copy_n(dst,
                                               src->u.object.items[i].key,
...
{
    size_t i;
    if (!object || object->type != TG3JSON_OBJECT)
        return NULL;
...
int tg3json_object_set_take_n(tg3json_value* object, const char* key, size_t key_len, tg3json_value* value)
{
    ...
    size_t cap;
    if (!object || object->type != TG3JSON_OBJECT || !value || !key)
        return 0;
    ...
    if (!tg3json__reserve_bytes((void**)&object->u.object.items,
                                sizeof(*object->u.object.items),
...
int tg3json_object_set_copy_n(tg3json_value* object, const char* key, size_t key_len, const tg3json_value* value)
{
    ...
    if (!tg3json_value_copy(&copy, value))
        return 0;
...
size_t tg3json_array_size(const tg3json_value* array)
{
    if (!array || array->type != TG3JSON_ARRAY)
        return 0;
...
{
    size_t cap;
    if (!array || array->type != TG3JSON_ARRAY || !value)
        return 0;
    cap = array->u.array.count;
    if (!tg3json__reserve_bytes((void**)&array->u.array.items,
                                sizeof(*array->u.array.items),
                                array->u.array.count + 1,
                                &cap))
    {
        return 0;
    }
...
static int tg3json__stringify_value_ex(tg3json__buffer* buf, const tg3json_value* value, int indent, int depth)
{
    size_t i;
    char numbuf[64];
    switch (value->type)
    {
        case TG3JSON_NULL: return tg3json__buf_append(buf, "null", 4);
        case TG3JSON_BOOL:
            return value->u.boolean ? tg3json__buf_append(buf, "true", 4) : tg3json__buf_append(buf, "false", 5);
        case TG3JSON_INT:
            ...
            for (i = 0; i < value->u.string.len; ++i)
            {
                ...
                switch (c)
                {
                    ...
                    case '\\':
                        ...
                        break;
                    case '\b':
                        if (!tg3json__buf_append(buf, "\\b", 2))
                            return 0;
                        break;
                    case '\f':
                        if (!tg3json__buf_append(buf, "\\f", 2))
                            return 0;
                        break;
                    case '\n':
                        ...
                        break;
                    case '\r':
                        if (!tg3json__buf_append(buf, "\\r", 2))
                            return 0;
                        break;
                    case '\t':
                        if (!tg3json__buf_append(buf, "\\t", 2))
                    ...
                    default:
                        if (c < 0x20u)
                        {
                            ...
                            numbuf[6] = '\0';
                            if (!tg3json__buf_append(buf, numbuf, 6))
                                return 0;
                        }
                        else
                        {
                            if (!tg3json__buf_putc(buf, (char)c))
                                return 0;
                        }
                        break;
                }
        ...
        case TG3JSON_ARRAY:
            ...
            for (i = 0; i < value->u.array.count; ++i)
            {
                if (i > 0 && !tg3json__buf_putc(buf, ','))
                    return 0;
                if (indent > 0 && !tg3json__indent(buf, indent, depth + 1))
                    return 0;
                if (!tg3json__stringify_value_ex(buf, &value->u.array.items[i], indent, depth + 1))
                    return 0;
            }
            if (indent > 0 && value->u.array.count > 0 && !tg3json__indent(buf, indent, depth))
                return 0;
            return tg3json__buf_putc(buf, ']');
        case TG3JSON_OBJECT:
            if (!tg3json__buf_putc(buf, '{'))
            ...
            for (i = 0; i < value->u.object.count; ++i)
            {
                ...
                if (!tg3json__stringify_value_ex(buf, &key_value, indent, depth + 1))
                    return 0;
                if (!tg3json__buf_putc(buf, ':'))
                    return 0;
                if (indent > 0 && !tg3json__buf_putc(buf, ' '))
                    return 0;
                if (!tg3json__stringify_value_ex(buf, value->u.object.items[i].value, indent, depth + 1))
                    return 0;
            }
            if (indent > 0 && value->u.object.count > 0 && !tg3json__indent(buf, indent, depth))
                return 0;
            return tg3json__buf_putc(buf, '}');
    }
    return 0; /* unreachable: all enum cases handled above. */
