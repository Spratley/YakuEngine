...
static uint8_t* tg3__decode_data_uri(tg3_arena* arena,
                                     const char* uri,
                                     uint32_t uri_len,
                                     size_t* out_len,
                                     char* out_mime,
...
static int32_t tg3__fs_read_file(uint8_t** out_data, uint64_t* out_size, const char* path, uint32_t path_len, void* ud)
{
    FILE* fp;
    long size;
    uint8_t* data;
...
static int tg3__json_has(const tg3json_value* o, const char* key) { return tg3json_object_get(o, key) ? 1 : 0; }
...
static int tg3__parse_string(tg3__parse_ctx* ctx,
...
static int tg3__parse_int(tg3__parse_ctx* ctx,
                          const tg3json_value* o,
                          const char* key,
                          int32_t* out,
                          int required,
...
static int tg3__parse_uint64(tg3__parse_ctx* ctx,
                             const tg3json_value* o,
                             const char* key,
                             uint64_t* out,
                             int required,
...
static int tg3__parse_double(tg3__parse_ctx* ctx,
                             const tg3json_value* o,
                             const char* key,
                             double* out,
                             int required,
...
static int tg3__parse_bool(tg3__parse_ctx* ctx,
                           const tg3json_value* o,
                           const char* key,
                           int32_t* out,
                           int required,
...
static tg3_value tg3__json_to_value(tg3__parse_ctx* ctx, const tg3json_value* j)
{
    ...
    switch (j->type)
    {
        case TG3JSON_NULL: v.type = TG3_VALUE_NULL; break;
        case TG3JSON_BOOL:
        ...
        case TG3JSON_OBJECT:
            ...
            {
                tg3_kv_pair* pairs =
                  (tg3_kv_pair*)tg3__arena_alloc(ctx->arena, j->u.object.count * sizeof(tg3_kv_pair));
                if (pairs)
                {
                    for (i = 0; i < j->u.object.count; ++i)
                    {
                        pairs[i].key =
                          tg3__arena_str(ctx->arena, j->u.object.items[i].key, (uint32_t)j->u.object.items[i].key_len);
...
static int tg3__load_external_file(tg3__parse_ctx* ctx,
                                   uint8_t** out_data,
                                   uint64_t* out_size,
                                   const char* uri,
                                   uint32_t uri_len)
{
    ...
    if (!ctx->opts.fs.read_file)
    {
        tg3__error_push(ctx->errors,
                        ...
                        "No filesystem callbacks available",
                        NULL,
                        -1);
        return 0;
    }
        ...
        if (tg3_is_data_uri(buf->uri.data, buf->uri.len))
        {
            ...
            if (!decoded && byte_length > 0)
            {
                tg3__error_push(ctx->errors,
                                ...
                                "Failed to decode buffer data URI",
                                NULL,
                                -1);
                return 0;
            }
            ...
            if (channels)
            {
                for (i = 0; i < count; ++i)
                {
                    ...
                    if (tg3__json_is_object(tgt_it))
                    {
                        ...
                        tg3__parse_string(ctx,
                                          tgt_it,
                                          "path",
                                          &channels[i].target.path,
                                          1,
...
#define TG3__CHECK_OPT(idx, max, path_str, fmt, ...) do { \
    ...
    for (i = 0; i < m->nodes_count && errs < TG3__IDX_ERR_CAP; ++i)
    {
        ...
        uint32_t k;
        TG3__CHECK_OPT(n->mesh,
                       m->meshes_count,
                       "/nodes",
                       "nodes[%u].mesh %d out of range [0,%u)",
                       i,
                       n->mesh,
                       m->meshes_count);
        TG3__CHECK_OPT(n->skin,
                       m->skins_count,
                       "/nodes",
                       "nodes[%u].skin %d out of range [0,%u)",
                       i,
                       n->skin,
                       m->skins_count);
        TG3__CHECK_OPT(n->camera,
                       ...
                       n->camera,
                       m->cameras_count);
        TG3__CHECK_OPT(n->light,
                       m->lights_count,
                       "/nodes",
                       "nodes[%u].light %d out of range [0,%u)",
                       i,
                       n->light,
                       m->lights_count);
...
static int tg3__valid_accessor_component_type(int32_t component_type)
{
    switch (component_type)
    {
        ...
        case TG3_COMPONENT_TYPE_UNSIGNED_INT:
        case TG3_COMPONENT_TYPE_FLOAT:          return 1;
        default:                                return 0;
    }
}
        ...
        if (a->sparse.is_sparse)
        {
            ...
            if (a->sparse.indices.buffer_view >= 0 && (uint32_t)a->sparse.indices.buffer_view < m->buffer_views_count)
            {
                ...
                if (idx_size_i <= 0 || tg3__u64_mul_overflow(sparse_count, (uint64_t)idx_size_i, &bytes)
                    || !tg3__validate_range_in_buffer_view(ctx,
                                                           bv,
                                                           a->sparse.indices.byte_offset,
                                                           bytes,
                                                           "sparse indices"))
                {
                    ok = 0;
...
static tg3_error_code tg3__parse_from_json(tg3__parse_ctx* ctx, const tg3json_value* json_doc, tg3_model* model)
{
    ...
    else if (ctx->opts.required_sections & TG3_REQUIRE_VERSION)
    {
        tg3__error_push(ctx->errors,
                        TG3_SEVERITY_ERROR,
                        TG3_ERR_MISSING_REQUIRED,
                        "Missing required 'asset' property",
                        "/",
                        -1);
    ...
    *bin_size_out = 0;
    if (size < 12)
    {
        tg3__error_push(errors,
                        TG3_SEVERITY_ERROR,
                        TG3_ERR_GLB_INVALID_HEADER,
                        "GLB data too small for header",
                        NULL,
                        -1);
    ...
    if ((uint64_t)total_length != size)
    {
        tg3__error_push(errors,
                        TG3_SEVERITY_ERROR,
                        TG3_ERR_GLB_SIZE_MISMATCH,
                        "GLB total length does not match data size",
                        NULL,
                        -1);
    ...
    while (offset + 8 <= (uint64_t)total_length)
    {
        ...
        if (offset + chunk_length > (uint64_t)total_length)
        {
            tg3__error_push(errors,
                            TG3_SEVERITY_ERROR,
                            TG3_ERR_GLB_CHUNK_ERROR,
                            "GLB chunk exceeds data size",
                            NULL,
                            -1);
...
static int tg3__serialize_uint64(tg3json_value* o,
                                 const char* key,
                                 uint64_t val,
...
static int tg3__serialize_double(tg3json_value* o, const char* key, double val, double default_val, int write_defaults)
{
...
static int tg3__serialize_bool(tg3json_value* o, const char* key, int32_t val, int32_t default_val, int write_defaults)
{
    if (val == default_val && !write_defaults)
        return 1;
...
static int tg3__value_to_json(const tg3_value* v, tg3json_value* out)
{
    ...
    switch (v->type)
    {
        case TG3_VALUE_NULL: return 1;
        case TG3_VALUE_BOOL: tg3json_value_init_bool(out, v->bool_val != 0); return 1;
        case TG3_VALUE_INT:  tg3json_value_init_int(out, v->int_val); return 1;
        case TG3_VALUE_REAL: tg3json_value_init_real(out, v->real_val); return 1;
...
static int tg3__serialize_asset(const tg3_asset* a, int wd, tg3json_value* out)
{
    ...
    if (!tg3__serialize_str(out, "version", a->version) || !tg3__serialize_str(out, "generator", a->generator)
        || !tg3__serialize_str(out, "minVersion", a->min_version) || !tg3__serialize_str(out, "copyright", a->copyright)
...
static int tg3__serialize_material(const tg3_material* m, int wd, tg3json_value* out)
{
    ...
    if (!tg3__serialize_double(out, "alphaCutoff", m->alpha_cutoff, 0.5, wd)
        || !tg3__serialize_bool(out, "doubleSided", m->double_sided, 0, wd) || !tg3__serialize_extras_ext(out, &m->ext))
        goto fail;
    return 1;
fail:
...
static int tg3__serialize_camera(const tg3_camera* c, int wd, tg3json_value* out)
{
    ...
    {
        tg3json_value_init_object(&tmp);
        if ((c->perspective.aspect_ratio > 0.0 && !tg3__json_set_real(&tmp, "aspectRatio", c->perspective.aspect_ratio))
...
static int tg3__serialize_scene(const tg3_scene* s, int wd, tg3json_value* out)
{
    (void)wd;
    tg3json_value_init_object(out);
    if (!tg3__serialize_str(out, "name", s->name) || !tg3__serialize_int_array(out, "nodes", s->nodes, s->nodes_count)
        || !tg3__serialize_extras_ext(out, &s->ext))
    {
        tg3json_value_free(out);
        return 0;
...
static int tg3__serialize_model(const tg3_model* model, int wd, int embed_images, int embed_buffers, tg3json_value* out)
{
    ...
    if (!tg3__serialize_string_array(out, "extensionsUsed", model->extensions_used, model->extensions_used_count)
        || !tg3__serialize_string_array(out,
                                        "extensionsRequired",
                                        model->extensions_required,
                                        model->extensions_required_count))
        goto fail;
...
#define TG3__SERIALIZE_ARRAY_FIELD(json_key, arr_ptr, arr_count, fn, ...) \
    ...
    TG3__SERIALIZE_ARRAY_FIELD("buffers",
                               model->buffers,
                               model->buffers_count,
                               tg3__serialize_buffer,
                               wd,
                               embed_buffers);
    TG3__SERIALIZE_ARRAY_FIELD("bufferViews",
                               model->buffer_views,
                               model->buffer_views_count,
...
#undef TG3__SERIALIZE_ARRAY_FIELD
    ...
    if (model->lights && model->lights_count > 0)
    {
        ...
        if (!tg3__json_set_take(&lights_ext, "lights", &lights) || !tg3__json_ensure_object(out, "extensions", &exts)
                                                 ...
                                                 const tg3_write_options* options)
{
    ...
    if (!tg3__serialize_model(model, options->serialize_defaults, options->embed_images, options->embed_buffers, &root))
    {
        tg3__error_push(errors,
                        TG3_SEVERITY_ERROR,
                        TG3_ERR_OUT_OF_MEMORY,
                        "Failed to serialize model to JSON tree",
                        NULL,
                        -1);
        return TG3_ERR_OUT_OF_MEMORY;
    }
